...
def _content(candidate): return f"{candidate.get('title','')} {candidate.get('source_text','')}"
def _title(candidate): return str(candidate.get("title") or "")
...
def _content_topics(candidate): return _topics(_content(candidate))
def _title_topics(candidate): return _topics(_title(candidate))
...
def suggest_category(candidate):
    ...
    content_text, title_text = _content(candidate), _title(candidate)
    ...
    # Local means explicit Sakhalin geography in headline/article, never publisher identity.
    if "local" in content:
    ...
    # Headline geography wins over publisher country.
    if "foreign" in title:
    ...
    # Economy needs a domestic signal. Global oil/markets must never be called Russia/economy.
    if "economy" in title or "economy" in content:
        ...
        if "russia" in title or _contains_any(title_text, DOMESTIC_ECON):
            return "ru_eco"
        if current == "ru_eco" and not _contains_any(title_text, DOMESTIC_ECON):
            return None
        return "ru_eco"
