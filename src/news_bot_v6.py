...
BAD_PHRASES = [
    ...
    "мошенники угрожали задержкой", "потеря 13,3 млн рублей",
]
...
SAKH_EVENTS = "дтп пожар происшествие землетрясение шторм циклон авария розыск погиб пострадал задержали суд мчс мвд отключение эвакуация".split()
POL = "кремль песков путин лавров мишустин госдума мид минобороны правительство президент совбез законопроект выборы kremlin putin lavrov moscow".split()
ECO = "цб центробанк ставка инфляция рубль бюджет минфин банк нефть газ экономика санкц экспорт импорт oil gas ruble inflation budget economy sanction".split()
RU_WORLD = "russia russian moscow kremlin ukraine ukrainian nato sanctions sanction putin lavrov china xi trump россия рф российск москва кремль украина украин нато санкции путин лавров китай си трамп".split()
...
LOW_NOISE = "спорт турнир матч рейтинг самокат фестиваль выставка конкурс".split()
LOCAL_NOISE = "афиша вакансии гороскоп погода реклама конкурс спорт".split()
...
SOURCES = [
    ("Sakhalin", "sakhalin", gnews("Сахалин OR Южно-Сахалинск OR Холмск OR Корсаков OR Курилы ДТП OR пожар OR происшествие OR землетрясение OR шторм OR авария OR розыск OR отключение", "ru", "RU")),
...
def resolve_google_url(url):
    ...
    try:
        ...
        final = r.url
        if final and not is_google(final):
            return final
    except Exception:
        pass
    return url
...
def classify(source_type, title, rss_text, page_desc, url):
    ...
    if source_type == "sakhalin" or has_terms(text, SAKH):
        if not has_terms(text, SAKH_EVENTS) and len(clean(rss_text + page_desc)) < 120:
            return None, 0
        return "📍 Сахалин", 100 + (8 if has_terms(text, SAKH_EVENTS) else 0)
    if source_type == "world":
        ...
        return None, 0
    if source_type == "it":
        return ("💻 IT / технологии", 72) if has_terms(text, IT) else (None, 0)
    if source_type == "ru":
        if "/moscow/" in path:
            return None, 0
        if has_terms(text, ECO):
            return "🇷🇺 Россия / экономика", 78
        if has_terms(text, POL) or has_terms(text, RU_WORLD):
            return "🇷🇺 Россия / политика", 76
        return None, 0
    return None, 0
...
def collect(state):
    ...
    for source, source_type, rss in SOURCES:
        ...
        for entry in feed.entries[:14]:
            ...
            source_text = " ".join(x for x in text_parts if x).strip()[:1800]
...
def select_order(items):
    # Модель только помогает расставить приоритет. Публикация всё равно идёт через валидатор.
    data = [{"id": x["id"], "category": x["category_hint"], "source": x["source"], "title": x["title"], "source_text": x["summary"][:700], "published_at": x["published_at"], "has_image": bool(x.get("image_file"))} for x in items]
    try:
        text = openrouter([
            {"role": "system", "content": "Ты выпускающий редактор. Верни только JSON."},
            {"role": "user", "content": "Расставь новости по приоритету для Telegram-канала: сначала Сахалин/важное ЧП, затем Мир о России/геополитика/IT. Не выбирай мусор, мелкие локальные московские криминальные заметки и новости без смысла. Верни JSON-массив id: [{\"id\":1},{\"id\":2}].\n" + json.dumps(data, ensure_ascii=False)},
        ], max_tokens=500)
        chosen = parse_json(text)
        ids = [int(x.get("id")) for x in chosen if str(x.get("id", "")).isdigit()]
    except Exception as exc:
        log("AI select fallback: " + str(exc))
        ids = []
    # Обязательный баланс: сначала один локальный, если есть.
    by_id = {x["id"]: x for x in items}
    local = [x["id"] for x in items if x["category_hint"] == "📍 Сахалин"]
    other = [x["id"] for x in items if x["category_hint"] != "📍 Сахалин"]
    balanced = []
    if local:
        balanced.append(local[0])
    for i in ids + other + local:
        if i in by_id and i not in balanced:
            balanced.append(i)
    return [by_id[i] for i in balanced]
...
def prompt_write(item, error=""):
    return (
        ...
        "6) Если фактов мало — верни JSON с reject=true.\n"
...
def validate(row, item):
    if row.get("reject") is True:
        return ["модель отклонила из-за малого количества фактов"]
...
def main():
    ...
    log("Сбор кандидатов")
    items = collect(state)
    log(f"Кандидатов после строгого фильтра: {len(items)}")
    if not items:
        save_state(state)
        return
